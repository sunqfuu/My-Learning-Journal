...
document.getElementById("navigation-menu").addEventListener("click", openNavMenu);
...
function getBlogRollHtml() {
    return blogposts.map((post) => {
        ...
        return `<div class="article-card">
                    <a href="#">
                        <img class="card-image" alt="${post.alt}" src="${post.image}" />
                        <time datetime="${post.date}">${new Intl.DateTimeFormat("en-US", options).format(publishDate)}</time>
                        <h2>${post.title}</h2>
                        <p>${post.subhed}</p>
                    </a>
                </div>`
